...
dat.UK <- read_csv("covid-19-cases-uk.csv")
```
...
last.date <- as.Date(dat.UK$Date[[nrow(dat.UK)]]) 

areas.alphabetical <- sort(unique(dat.UK$Area)) # put areas in alphabetical order, for saving together
...
area.incidence <- lapply(areas.alphabetical, function(area) {
  dat.area <- dat.UK[dat.UK$Area==area,]
